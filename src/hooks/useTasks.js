...
const useTasks = () => {
  ...
  const getTasks = async () => {
    ...
    const data = await apiRequest('GET', "https://192.168.9.119:8000/tasks", null, isValidToken);
  ...
  const updateTask = async (task, taskID, isStatusUpdate) => {
    ...
    if (taskID) {
      const data = await apiRequest('PATCH', `https://192.168.9.119:8000/tasks/update/${taskID}`, task, isValidToken);
      console.log(data);
      if (isStatusUpdate) {
  ...
  const deleteCompletedTasks = async () => {
    const isValidToken = await fetchUserData()
    try {
      const data = await apiRequest('DELETE', "https://192.168.9.119:8000/tasks/deleteCompleted", null, isValidToken);
      console.log(data);
      await getTasks();
    } catch (error) {
      console.log(error);
  ...
  const addTask = async () => {
    ...
    if (taskBody.text !== undefined) {
      const data = await apiRequest('POST', "https://192.168.9.119:8000/tasks", taskBody, isValidToken);
  ...
  const handleDeleteClick = async (taskID) => {
    ...
    const data = await apiRequest('DELETE', `https://192.168.9.119:8000/tasks/delete/${taskID}`,null, isValidToken);
    console.log(data);
  ...
  const searchUsers = async (search) => {
    const isValidToken = await fetchUserData()
    if (search !== '') {
      const data = await apiRequest('GET', `https://192.168.9.119:8000/users/usersList?search=${search}`, null, isValidToken);
