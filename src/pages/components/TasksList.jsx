      ...
      <Box sx={{ mt: 3, ml: 2, width: "100%" }}>  
        {searchedTasks &&
          searchedTasks.map((task, index) => (
            <div key={task._id}>
              <Grid container onMouseEnter={() => setShowDeleteIcon(index)}
                ...
                >
                  ...
                  <Grid xs={loggedUser && loggedUser.role == 'user' ? 7 : 5} sm={loggedUser && loggedUser.role == 'user' ? 9 : 5.5} sx={{ overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%" }}
                  ...
                  <Grid xs={loggedUser && loggedUser.role == 'user' ? 4 : 3} sm={loggedUser && loggedUser.role == 'user' ? 2 : 4}>
                    {loggedUser && loggedUser.role == 'admin' ? (<Typography sx={{ fontSize: { xs: '8px', sm: '15px' }, fontWeight: 'bold', overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%", marginLeft: {sm: 7} }}>{searchedTasks[index].username}</Typography>) : (<div></div>)}
                  </Grid>
                </Grid>
                <Grid xs={1} sx={{ display: "flex", justifyContent: "center" }}>
